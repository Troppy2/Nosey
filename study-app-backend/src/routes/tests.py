...
async def create_test(
    ...
    try:
        ...
        if not title or not test_type or not notes_files:
            raise StudyAppException("title, test_type, and at least one notes_files document are required")
        if len(notes_files) > MAX_UPLOAD_DOCUMENTS:
            raise StudyAppException(f"You can upload at most {MAX_UPLOAD_DOCUMENTS} documents")
        valid_files = [notes_file for notes_file in notes_files if isinstance(notes_file, UploadFile)]
