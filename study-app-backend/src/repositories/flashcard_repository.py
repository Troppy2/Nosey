...
class FlashcardRepository(BaseRepository[Flashcard]):
    async def create(self, folder_id: int, front: str, back: str, source: str | None) -> Flashcard:
    ...
    async def list_with_stats(self, folder_id: int, user_id: int) -> list[tuple[Flashcard, int, int, float | None, object]]:
        ...
        stmt: Select[tuple[Flashcard, int, int, float | None, object]] = (
            select(
                Flashcard,
    ...
    async def list_with_stats_for_user(self, user_id: int) -> list[tuple[Flashcard, int, int, float | None, object]]:
        correct_count = func.sum(case((FlashcardAttempt.correct.is_(True), 1), else_=0))
        total_count = func.count(FlashcardAttempt.id)
        stmt: Select[tuple[Flashcard, int, int, float | None, object]] = (
            select(
    ...
    async def record_attempt(
        self, user_id: int, flashcard_id: int, correct: bool, time_ms: int | None
    ) -> FlashcardAttempt:
